...
void Dfs(int v, double& dist, const std::vector<geometry::Vector2D>& points,
        std::vector<bool>& visited, std::vector<int>& path, std::vector<int>& result, double& best_dist) {
    visited[v] = true;
    path.emplace_back(v);
...
std::vector<int> BruteHamiltonial(const std::vector<geometry::Vector2D>& points) {
    ...
    assert(std::unordered_set<int>(result.begin(), result.end()).size() == result.size());
