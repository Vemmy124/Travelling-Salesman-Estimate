...
geometry::Graph ExtractGraph(const geometry::Triangulation& triangulation,
...
double GetDistance(const geometry::Vector2D& fst,
                   const geometry::Vector2D& snd) {
...
std::vector<int> GetHamiltonialCycle(std::vector<Vector2D>& points) {
  auto triangulation_solver =
      geometry::DelaunayBuilder::Create(std::move(points));
  ...
  assert(std::unordered_set<int>(sequence.begin(), sequence.end()).size()
             == sequence.size());
